        ...
        <>
            <style>{`
                ...
                *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
                ...
                .add-btn { background: linear-gradient(135deg, #10b981, #059669); color: #fff; border: none; border-radius: 12px; padding: 11px 20px; font-size: 13px; font-weight: 700; cursor: pointer; font-family: inherit; box-shadow: 0 6px 20px rgba(16,185,129,0.35); transition: all 0.2s; display: flex; align-items: center; gap: 7px; white-space: nowrap; }
                ...
                .search-wrap { position: relative; margin-bottom: 18px; max-width: 380px; }
                ...
                .search-input:focus { border-color: #10b981; background: rgba(16,185,129,0.05); }
                ...
                thead { background: rgba(16,185,129,0.07); }
                th { padding: 12px 16px; text-align: left; font-size: 10px; font-weight: 700; color: #10b981; text-transform: uppercase; letter-spacing: 0.8px; border-bottom: 1px solid rgba(255,255,255,0.05); }
                ...
                .b-indigo { background: rgba(99,102,241,0.12); color: #a5b4fc; border: 1px solid rgba(99,102,241,0.2); }
                ...
                .cupboard-icon { width: 38px; height: 38px; border-radius: 10px; background: rgba(16,185,129,0.12); border: 1px solid rgba(16,185,129,0.2); display: flex; align-items: center; justify-content: center; font-size: 18px; flex-shrink: 0; }
                .place-icon { width: 38px; height: 38px; border-radius: 10px; background: rgba(99,102,241,0.12); border: 1px solid rgba(99,102,241,0.2); display: flex; align-items: center; justify-content: center; font-size: 18px; flex-shrink: 0; }
                ...
                .field-input-m:focus { border-color: #10b981; background: rgba(16,185,129,0.05); }
                ...
                .submit-btn-m { flex: 1; padding: 12px; background: linear-gradient(135deg, #10b981, #059669); color: white; border: none; border-radius: 10px; font-size: 13px; font-weight: 700; cursor: pointer; font-family: inherit; box-shadow: 0 6px 20px rgba(16,185,129,0.3); transition: all 0.2s; }
            ...
            <div className="page-root">
                <aside className="sidebar">
                    ...
                    {navItems.map(item => (
                        <Link key={item.href} href={item.href} className={`nav-link${item.href === '/dashboard/storage' ? ' active' : ''}`}>
                            <div className="nav-icon-wrap" style={{ background: item.href === '/dashboard/storage' ? 'rgba(99,102,241,0.2)' : 'rgba(255,255,255,0.05)' }}>{item.icon}</div>
                ...
                <main className="main-content fade-in">
                    ...
                    <div className="stats-row">
                        {[
                            { label: 'Cupboards', count: cupboards.length, color: '#34d399', bg: 'rgba(16,185,129,0.12)' },
                            { label: 'Places',    count: places.length,    color: '#a5b4fc', bg: 'rgba(99,102,241,0.12)' },
                        ].map((s, i) => (
                    ...
                    {activeTab === 'cupboards' ? (
                        <div className="table-wrap">
                            <table>
                                <thead><tr>{['Cupboard', 'Location', 'Places', 'Actions'].map(h => <th key={h}>{h}</th>)}</tr></thead>
                                <tbody>
                                    {filteredCupboards.length === 0 ? (
                                        <tr><td colSpan={4}><div className="empty-state"><div className="empty-icon">🗄️</div><p>No cupboards yet — add your first one!</p></div></td></tr>
                                    ) : filteredCupboards.map(c => (
                                        <tr key={c.id}>
                                            <td><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><div className="cupboard-icon">🗄️</div><span className="td-primary">{c.name}</span></div></td>
                                            <td style={{ color: '#64748b' }}>📍 {c.location || '—'}</td>
                                            <td><span className="badge b-indigo">{places.filter(p => p.cupboard_id === c.id).length} places</span></td>
                                            <td><div style={{ display: 'flex', gap: '7px' }}><button className="action-btn edit-btn" onClick={() => openCupboardModal(c)}>✏️ Edit</button><button className="action-btn del-btn" onClick={() => handleDelete('cupboard', c.id)}>🗑️</button></div></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                        ...
                        <div className="table-wrap">
                            <table>
                                <thead><tr>{['Place', 'Cupboard', 'Actions'].map(h => <th key={h}>{h}</th>)}</tr></thead>
                                <tbody>
                                    {filteredPlaces.length === 0 ? (
                                        <tr><td colSpan={3}><div className="empty-state"><div className="empty-icon">📍</div><p>No places yet — add your first one!</p></div></td></tr>
                                    ) : filteredPlaces.map(p => (
                                        <tr key={p.id}>
                                            <td><div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}><div className="place-icon">📍</div><span className="td-primary">{p.name}</span></div></td>
                                            <td><span className="badge b-green">🗄️ {p.cupboard?.name || '—'}</span></td>
                                            <td><div style={{ display: 'flex', gap: '7px' }}><button className="action-btn edit-btn" onClick={() => openPlaceModal(p)}>✏️ Edit</button><button className="action-btn del-btn" onClick={() => handleDelete('place', p.id)}>🗑️</button></div></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                ...
                <div className="modal-overlay">
                    <div className="modal">
                        <div className="modal-header">
                            <h3 className="modal-title">{editItem ? `✏️ Edit ${modalType === 'cupboard' ? 'Cupboard' : 'Place'}` : `➕ New ${modalType === 'cupboard' ? 'Cupboard' : 'Place'}`}</h3>
                        ...
                        <div className="form-fields">
                            {modalType === 'cupboard' ? (
                                <>
                                    <div><label className="field-label-m">Cupboard Name *</label><input type="text" value={cupboardForm.name} className="field-input-m" placeholder="e.g. Cabinet A" onChange={e => setCupboardForm({ ...cupboardForm, name: e.target.value })} /></div>
                                    <div><label className="field-label-m">Location</label><input type="text" value={cupboardForm.location} className="field-input-m" placeholder="e.g. Lab Room 1" onChange={e => setCupboardForm({ ...cupboardForm, location: e.target.value })} /></div>
                                </>
                            ) : (
                                <>
                                    <div><label className="field-label-m">Place Name *</label><input type="text" value={placeForm.name} className="field-input-m" placeholder="e.g. Shelf A1" onChange={e => setPlaceForm({ ...placeForm, name: e.target.value })} /></div>
                                    <div>
                                        <label className="field-label-m">Cupboard *</label>
                                        <select value={placeForm.cupboard_id} className="field-input-m" onChange={e => setPlaceForm({ ...placeForm, cupboard_id: e.target.value })}>
                                            <option value="">Select cupboard...</option>
                                            {cupboards.map(c => <option key={c.id} value={c.id}>{c.name}{c.location ? ` — ${c.location}` : ''}</option>)}
                                        </select>
                                    </div>
                                </>
                            )}
