...
export default function BorrowingsPage() {
    ...
    const [borrowings, setBorrowings] = useState<any[]>(() => {
        if (typeof window !== 'undefined') {
            try { return JSON.parse(localStorage.getItem('cache_borrowings') || '[]'); }
            catch { return []; }
        }
        return [];
    });
    ...
    const fetchAll = async () => {
        try {
            ...
            localStorage.setItem('cache_borrowings', JSON.stringify(b.data));
            setItems(i.data.filter((item: any) => item.quantity > 0));
            localStorage.setItem('cache_items', JSON.stringify(i.data));
    ...
    const filtered = borrowings.filter((b: any) =>
        b.borrower_name?.toLowerCase().includes(search.toLowerCase()) ||
        b.item?.name?.toLowerCase().includes(search.toLowerCase())
    );

    const activeCount   = borrowings.filter((b: any) => b.status === 'active').length;
    const returnedCount = borrowings.filter((b: any) => b.status === 'returned').length;
        ...
        <>
            <style>{`
                ...
                *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
                ...
                .add-btn { background: linear-gradient(135deg, #f59e0b, #d97706); color: #fff; border: none; border-radius: 12px; padding: 11px 20px; font-size: 13px; font-weight: 700; cursor: pointer; font-family: inherit; box-shadow: 0 6px 20px rgba(245,158,11,0.3); transition: all 0.2s; display: flex; align-items: center; gap: 7px; white-space: nowrap; }
                ...
                th { padding: 12px 16px; text-align: left; font-size: 10px; font-weight: 700; color: #f59e0b; text-transform: uppercase; letter-spacing: 0.8px; border-bottom: 1px solid rgba(255,255,255,0.05); }
                ...
                .return-btn { background: rgba(16,185,129,0.12); color: #34d399; border: none; padding: 5px 12px; border-radius: 8px; font-size: 12px; font-weight: 700; cursor: pointer; font-family: inherit; transition: all 0.15s; }
                ...
                .field-input-m:focus { border-color: #f59e0b; }
                ...
                .submit-btn-m { flex: 1; padding: 12px; background: linear-gradient(135deg, #f59e0b, #d97706); color: white; border: none; border-radius: 10px; font-size: 13px; font-weight: 700; cursor: pointer; font-family: inherit; }
            ...
            <div className="page-root">
                <aside className="sidebar">
                    ...
                    {navItems.map(item => (
                        <Link key={item.href} href={item.href} className={`nav-link${item.href === '/dashboard/borrowings' ? ' active' : ''}`}>
                            <div className="nav-icon-wrap" style={{ background: item.href === '/dashboard/borrowings' ? 'rgba(99,102,241,0.2)' : 'rgba(255,255,255,0.05)' }}>{item.icon}</div>
                ...
                <main className="main-content fade-in">
                    <div className="page-header">
                        ...
                        <button className="add-btn" onClick={() => { setError(''); setShowModal(true); }}>
                            ➕ New Borrowing
                        </button>
                    </div>
                    ...
                    <div className="stats-row">
                        {[
                            { label: 'Active',   count: activeCount,   color: '#fbbf24', bg: 'rgba(251,191,36,0.1)' },
                            { label: 'Returned', count: returnedCount, color: '#4ade80', bg: 'rgba(74,222,128,0.1)' },
                            { label: 'Overdue',  count: overdueCount,  color: '#f87171', bg: 'rgba(248,113,113,0.1)' },
                        ].map((s, i) => (
                    ...
                    <div className="search-wrap">
                        ...
                        <input type="text" placeholder="Search by borrower or item..." className="search-input" value={search} onChange={e => setSearch(e.target.value)} />
                    </div>

                    <div className="table-wrap">
                        <table>
                            <thead>
                                <tr>{['Item', 'Borrower', 'Contact', 'Qty', 'Borrow Date', 'Expected Return', 'Status', 'Action'].map(h => <th key={h}>{h}</th>)}</tr>
                            </thead>
                            <tbody>
                                {filtered.length === 0 ? (
                                    <tr><td colSpan={8}><div className="empty-state"><div style={{ fontSize: '40px', marginBottom: '10px', opacity: 0.35 }}>📋</div><p>{search ? 'No results found' : 'No borrowings recorded yet'}</p></div></td></tr>
                                ) : filtered.map((b: any) => {
                        ...
                        <div className="form-fields">
                            <div>
                                ...
                                <select value={form.item_id} className="field-input-m" onChange={e => setForm({ ...form, item_id: e.target.value })}>
                                    ...
                                    {items.map((i: any) => <option key={i.id} value={i.id}>{i.name} (Qty: {i.quantity})</option>)}
                                </select>
                            </div>
                            <div>
