        ...
        <>
            <style>{`
                ...
                *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
                ...
                .search-input { width: 100%; padding: 11px 16px 11px 42px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; font-size: 13px; color: #e2e8f0; outline: none; font-family: inherit; }
                ...
                .loading-state { display: flex; align-items: center; justify-content: center; padding: 80px; color: #475569; font-size: 14px; }
                ...
                .close-btn { background: rgba(255,255,255,0.07); border: none; border-radius: 10px; width: 32px; height: 32px; cursor: pointer; color: #94a3b8; font-size: 16px; }
                ...
                .field-input-m { width: 100%; padding: 11px 14px; font-size: 13px; font-family: inherit; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); border-radius: 10px; outline: none; color: #e2e8f0; box-sizing: border-box; }
                ...
                .submit-btn-m { flex: 1; padding: 13px; background: linear-gradient(135deg, #8b5cf6, #7c3aed); color: white; border: none; border-radius: 12px; font-size: 14px; font-weight: 700; cursor: pointer; font-family: inherit; }
                .cancel-btn-m { flex: 1; padding: 13px; background: rgba(255,255,255,0.06); color: #94a3b8; border: 1px solid rgba(255,255,255,0.1); border-radius: 12px; font-size: 14px; font-weight: 600; cursor: pointer; font-family: inherit; }
            `}</style>
            <div className="page-root">
                <aside className="sidebar">
                    ...
                    <div className="user-chip">
                        ...
                        <div><div className="user-name">{user?.name}</div><span className="user-role">{user?.role}</span></div>
                    </div>
                ...
                <main className="main-content">
                    ...
                    <div className="stats-row">
                        {[
                            { label: 'Total Users', count: users.length,                                 color: '#c4b5fd', bg: 'rgba(139,92,246,0.1)' },
                            { label: 'Admins',      count: users.filter(u => u.role === 'admin').length, color: '#a5b4fc', bg: 'rgba(99,102,241,0.1)' },
                        ...
                        <div className="table-wrap">
                            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                                <thead className="table-head">
                                    <tr>{['User', 'Email', 'Role', 'Joined', 'Actions'].map(h => <th key={h} className="th">{h}</th>)}</tr>
                                </thead>
                                <tbody>
                                    {filtered.length === 0 ? (
                                        <tr><td colSpan={5}><div className="empty-state"><div style={{ fontSize: '48px' }}>👤</div><p style={{ marginTop: '12px' }}>No users found</p></div></td></tr>
                                    ) : filtered.map((u, idx) => (
                                        <tr key={u.id} className="tr">
                                            ...
                                            <td className="td"><span className={`badge ${u.role === 'admin' ? 'b-purple' : 'b-blue'}`}>{u.role === 'admin' ? '👑 Admin' : '👤 Staff'}</span></td>
                                            <td className="td" style={{ fontSize: '12px', color: '#475569' }}>{u.created_at ? new Date(u.created_at).toLocaleDateString() : '—'}</td>
                                            <td className="td">{u.id === user?.id ? <span className="you-badge">✅ You</span> : <button className="action-btn del-btn" onClick={() => handleDelete(u.id)}>🗑️ Delete</button>}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
