...
export default function UsersPage() {
    ...
    // ✅ Load from cache instantly instead of starting empty
    const [users, setUsers] = useState<any[]>(() => {
        if (typeof window !== 'undefined') {
            try { return JSON.parse(localStorage.getItem('cache_users') || '[]'); }
            catch { return []; }
        }
        return [];
    });
    ...
    const fetchUsers = async () => {
        try {
            ...
            setUsers(res.data);
            // ✅ Save fresh data to cache after fetching
    ...
    // ✅ True loading = API still fetching AND no cached data to show
    const isColdLoading = loading && users.length === 0;
        ...
        <>
            <style>{`
                ...
                *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
                ...
                .refreshing-bar { height: 2px; background: linear-gradient(90deg, #8b5cf6, #6366f1, #8b5cf6); background-size: 200% 100%; animation: shimmer 1.4s infinite; border-radius: 2px; margin-bottom: 10px; }
                @keyframes shimmer { 0% { background-position: 200% 0; } 100% { background-position: -200% 0; } }
                .stale-badge { font-size: 11px; color: #475569; margin-bottom: 14px; display: flex; align-items: center; gap: 5px; }
            ...
            <div className="page-root">
                ...
                <main className="main-content fade-in">
                    ...
                    {/* ✅ Thin animated bar while refreshing in background (cache already shown) */}
                    {loading && users.length > 0 && <div className="refreshing-bar" />}

                    {/* ✅ Subtle hint that data is from cache while API wakes up */}
                    {loading && users.length > 0 && (
                        <p className="stale-badge">⏳ Syncing latest data from server...</p>
                    )}
                    ...
                    <div className="table-wrap">
                        <table>
                            ...
                            <tbody>
                                {/* ✅ Only show full spinner when there's truly nothing to display */}
                                {isColdLoading ? (
